...
# [Implementation 4] 세 join 알고리즘의 결과를 bag 단위로 비교합니다.
# 중복, NULL, 빈 입력, 치우친 key 분포에서도 nested-loop 기준 결과와 같은지 확인합니다.
class JoinAlgorithmTests(unittest.TestCase):
    ...
    def test_randomized_algorithms_match_nested_loop_bag(self) -> None:
        randomizer = random.Random(2026)
        for case in range(100):
            left = [
                {"id": index, "k": randomizer.choice([None, -1, 0, 1, 2, 2])}
                for index in range(randomizer.randrange(0, 20))
            ]
            right = [
                {"id": 1000 + index, "k": randomizer.choice([None, -1, 0, 1, 2, 2])}
                for index in range(randomizer.randrange(0, 20))
            ]
            expected = normalize(nested_loop_join(left, right, "k", "k"))
            with self.subTest(case=case):
                self.assertEqual(normalize(hash_join(left, right, "k", "k")), expected)
                self.assertEqual(normalize(merge_join(left, right, "k", "k")), expected)
