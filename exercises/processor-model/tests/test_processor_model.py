...
from processor_model import (  # noqa: E402
    bits,
    cache,
    coherence,
    control,
...
# [Implementation 12] 전체 회귀 검사
# 산술, 파이프라인 위험, 주소 변환과 일관성의
# 핵심 조건이 함께 유지되는지 검사합니다.
...
class PerformanceTests(unittest.TestCase):
    def test_cpu_time(self) -> None:
        result = perf.cpu_time(1_000_000_000, 2.0, 2.0)
...
class IsaTests(unittest.TestCase):
    ...
    def test_wraparound_and_zero_register(self) -> None:
        ...
        self.assertEqual(result["registers"]["r1"], -2_147_483_648)
...
class PipelineTests(unittest.TestCase):
    def _trace(self, name: str) -> list[isa.Instruction]:
        return isa.parse_pipeline_trace(
            (ROOT / f"fixtures/traces/{name}").read_text(encoding="utf-8").splitlines()
        )
...
class CacheTests(unittest.TestCase):
    def test_three_c_classification_sums_to_misses(self) -> None:
        accesses = cache.parse_trace(
            (ROOT / "fixtures/traces/cache.trace").read_text(encoding="utf-8").splitlines()
        )
        result = cache.CacheSimulator(16, 4, 1).run(accesses)
        classified = (
            result["compulsory_misses"]
            + result["conflict_misses"]
            + result["capacity_misses"]
        )
...
class VirtualMemoryTests(unittest.TestCase):
    def test_tlb_fault_permission_and_invalidation(self) -> None:
        ...
        mappings = {
            int(vpn, 0): vm.Mapping(entry["pfn"], set(entry["permissions"]))
            for vpn, entry in config["mappings"].items()
        }
        operations = vm.parse_operations(
            (ROOT / "fixtures/vm/trace.txt").read_text(encoding="utf-8").splitlines()
        )
...
class CoherenceTests(unittest.TestCase):
    def test_false_sharing_causes_invalidations(self) -> None:
        accesses = coherence.parse_trace(
            (ROOT / "fixtures/traces/coherence-false-sharing.trace")
...
class BranchPredictorTests(unittest.TestCase):
    def test_counter_learns_repeated_taken_branch(self) -> None:
        ...
        predictions = [model.update(0x100, True) for _ in range(4)]
    ...
    def test_aliasing_is_deterministic(self) -> None:
        ...
        model.update(0x00, True)
        self.assertTrue(model.predict(0x08))
